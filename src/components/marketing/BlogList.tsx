...
export function BlogList() {
  ...
  if (!posts.length) {
    return (
      <div className="rounded-[1.5rem] border border-dashed border-line bg-paper-2/60 px-6 py-16 text-center">
    ...
    <div className="grid gap-5 md:grid-cols-2">
      {posts.map((post) => (
        <Link
          key={post._id}
          href={`/blog/${post.slug}/`}
          className="magnetic-card group overflow-hidden rounded-[1.5rem] border border-line bg-ns-white"
        >
          {post.image?.url ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={post.image.url} alt="" className="h-48 w-full object-cover" />
          ) : (
            <div className="flex h-48 items-end bg-paper-2 p-6">
              {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="p-6">
            ...
            <p className="mt-3 text-sm font-semibold text-accent transition group-hover:translate-x-1">
              Read
            </p>
          </div>
        </Link>
