...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  return (
    <>
      <section className="ns-mesh ns-grain relative pt-[7.5rem] pb-16 md:pt-[9rem] md:pb-20">
        <div className="relative z-[2] container-ns">
          <Reveal>
            ...
            <h1 className="font-display mt-5 max-w-4xl text-4xl leading-[1.08] tracking-tight text-ink text-balance sm:text-5xl md:text-7xl">
              {service.title}
            </h1>
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-ink-soft text-pretty sm:text-lg md:text-xl">
              {service.hero}
            </p>
      ...
      <section className="border-y border-line bg-ns-white py-6 overflow-x-clip">
      ...
      <section className="py-24 md:py-32">
        <div className="container-ns grid gap-14 lg:grid-cols-[1.1fr_0.9fr] lg:gap-20">
          <Reveal variant="left">
            ...
            <ul className="mt-8 space-y-3">
              {service.highlights.map((item) => (
                <li
                  key={item}
                  className="flex gap-3 rounded-2xl border border-transparent bg-paper-2/70 px-4 py-3.5 text-sm text-ink-soft transition hover:border-accent/30 hover:bg-ns-white md:text-base"
                >
          ...
          <Reveal variant="scale" delayMs={120}>
            <div className="magnetic-card rounded-[1.75rem] border border-white/10 bg-void px-8 pt-11 pb-9 text-on-void md:px-11 md:pt-14 md:pb-11">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {service.process.map((step, i) => (
              <Reveal key={step.step} delayMs={i * 80}>
                <article className="magnetic-card group h-full rounded-[1.5rem] border border-line bg-ns-white px-8 pt-11 pb-9 md:px-11 md:pt-14 md:pb-11">
          ...
          <Reveal>
            ...
            <ol className="mt-8 space-y-0">
              {service.deliverables.map((item, i) => (
                <li
                  key={item}
                  className="interactive-row flex items-start gap-4 border-b border-line py-6"
                >
                  <span className="font-mono text-xs tabular-nums tracking-wide text-muted">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="min-w-0 text-base text-ink md:text-lg">{item}</span>
                </li>
              ))}
            </ol>
          </Reveal>
      ...
      <section className="border-t border-line bg-void py-20 text-on-void md:py-24">
